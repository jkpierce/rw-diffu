...
\label{sec:appendixC}
The easiest approach to extract the earlier results of \cite{Wu2019}, \citet{Lisle1998}, and \citet{Einstein1937} for the moments and positional variance as limiting cases,it's most straightfoward to use the Laplace transformed moments \ref{eq:lapmean} and \ref{eq:lapsecondmom} as a starting point.
First we obtain the \citet{Wu2019} model. 
...
To obtain their conclusions on bedload diffusion, we send the mean duration of motion to zero while sending the velocity to infinity ($1/k_2\rightarrow 0$ and $v\rightarrow \infty$) and holding the mean step distance $l = v/k_2$ constant.
...
This provides 
\begin{align}
\bra \tilde{x} \ket &= v \frac{\theta_2 s + k_1}{s(s+k')}, \label{eq:li1}\\
\bra \tilde{x}^2 \ket &= 2v^2 \frac{(s+k_1)(\theta_2s+k_1)}{s^3(s+k')^2}. \label{eq:li2}
\end{align}
Inverting these equations provides a variance encoding two stages of diffusion: at small times there is super-diffusion $\sigma_x^2 \propto t^\gamma$ with $\gamma = 2-3$ depending on the initial condition, and at longer times there is normal diffusion. These equations are lengthy and are not included here, nor were they actually provided in the \citet{Lisle1998} work, as it concentrated on the asymptotic solution in the $t\rightarrow \infty$ limit when $\theta_2=1$. For the sake of completeness we have included a complete analysis of the \citet{Lisle1998} model and its diffusion characteristics in the supplementary information. There we show that equations \ref{eq:li1} and \ref{eq:li2} provide two stages of diffusion and link back to the Einstein model in the limit of instantaneous steps: $1/k_2 \rightarrow 0$ and $v\rightarrow \infty$ while $v/k_2 = l$.

Finally, we examine the limit $t\rightarrow 0 $. This will highlight the effect of initial conditions on the local range super-diffusion.
...
\end{align}
Taking only leading terms, these equations provide the asymptotic ($t\rightarrow 0$) variance 
...
\acknowledgments
J. Pierce acknowledges helpful exchanges with Eduardo Daly and Peter H{\"a}nggi during the early stages of this work. He would like to thank Mindy Saunders and Leonardo Golubovic for their careful guidance in mathematics through the years. M. Hassan is supported by an NSERC Discovery grant. The Python simulation code is available at \sloppy
