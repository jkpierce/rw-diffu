...
These equations can be inverse transformed using the linearity, translation, integration, and differentiation properties of Laplace transforms \citep[e.g.][]{Arfken1985} after expanding them in partial fractions.
For the mean, the calculation is
\begin{align}
\frac{2b}{v}\bra x \ket &= \big[\theta_2 + (k_1+\theta_2 \kappa)\int_0^t dt\circ\big]\El^{-1}\Big\{ \frac{1}{s+a-b}-\frac{1}{s+a+b};t\Big\}\\
...
\end{align}
The symbol $\circ$ denotes operation on everything to the right of the following parenthesis. This equation rearranges to (\ref{eq:mean}).
