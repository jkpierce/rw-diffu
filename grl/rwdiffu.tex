...
The probability that a sojourn in this state lasts for at least a time $t$ is then given by $\Psi_1(t) = \int_t^\infty \psi_1(t)dt = e^{-k_1 t}$.
...
% diffusion shows three ranges and a range of no diffusion; relies on k2>k1>kap and initial condition theta_1=1.
% these assumptions seem reasonable in light of experimental data
% the timescales of these ranges can be reasoned physically. dominant scales are 1/k1, 1/k2, 1/kap. crossovers between the ranges are governed by mixtures of these scales
% implication is that normal diffusion should be present for a range of times $T_L<t<T_I$


The key inputs of the model are the velocity $v$ of moving sediment and three key timescales: $1/k_2$, $1/k_1$, and $1/\kappa$.
These are the mean duration of motion, the mean duration of rest, and the mean time sediment can rest before becoming buried.
When $k_2\gg k_1 \gg \kappa$ and $\theta_1=1$, meaning motions are much shorter than rests and that grains start from rest, as displayed in figure (\ref{fig:var}), the model expresses three ranges of diffusion where the positional variance scales approximately as a power law: $\sigma_x^2 \propto t^\gamma$.
These are identified as the local, intermediate, and global ranges proposed by \citet{Nikora2001a, Nikora2002}.
There is also a fourth range where there is no diffusion, corresponding to the burial of all sediment grains.
In between these ranges, there are crossover regions where the scaling is transitional.
Because these crossovers are relatively narrow, spanning less than a decade of time units, we can characterize them approximately using a crossover time concept.
As depicted on figure (\ref{fig:var}), the three crossovers are at the times $T_L$ (local/intermediate crossover), $T_I$ (intermediate/global crossover), and $T_G$ (global/geomorphic crossover).

We determine expressions for these crossover times by reasoning about the mechanism generating each diffusion range with the underlying timescales $1/k_1$, $1/k_2$, and $1/\kappa$ of the model.



 This aligns with the original concept of \citet{Nikora2001a,Nikora2002} and with a variety of bedload transport numerical simulations \citep{Bialik2012}. The model also expresses a period of no diffusion that appears at timescales larger than the global range.
Because this period involves the burial of sediment grains, its properties should be dependent on scour and fill dynamics within the channel.
As seen in figure (\ref{fig:var}), between the diffusion ranges, there are relatively narrow cross-over regions in which the scaling does not follow a pure power law.
Since the crossover regions are narrow, they can be characterized by a set of crossover times.
As indicated in figure (\ref{fig:var}), these are denoted $T_L$, $T_I$, and $T_G$ for local, intermediate, and global. 
These timescales can be written in terms of the fundamental scales of sediment motion: $1/k_2$, the mean time spent in motion; $1/k_1$, the mean time spent at rest; and $1/\kappa$, the mean time required for a resting grain to become buried.

The local range is associated with 




There has been increasing emphasis on the timescales of sediment burial and scour to a particular elevation \citep{Nakagawa1980, Voepel2013, Martin2014, Pierce2019}.
...
% limits to einstein/wu/lisle
% leveraging approaches from the physics literature on random walks, we present a unifying mathematical framework encompassing the existing sediment diffusion models
% a longstanding and unsolved issue in sediment diffusion is the effect of morphology.
% shortcomings of the model include inability to incorporate unsteady transport conditions, and the longstanding problem of morphology.
% we have identified simplified processes of sediment motion in river channels: entrainment, deposition, and burial.
% However, a whole host of processes plays a role in real sediment diffusion processes.
% the effect of morphology on transport is a well-known issue.
%
