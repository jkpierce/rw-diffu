...
\begin{keypoints}
\item We develop a random-walk model of particles in intermittent transport through an environment with traps.
\item Its solution provides three diffusion ranges of which two are anomalous.
\item We apply it to bedload transport through rivers, generalizing the classic approach of H.A. Einstein and clarifying the scale-dependence of the process.
...
In gravel-bed rivers, grains transport downstream in sequences of rests and motions. While resting, they can be trapped by burial.
...
If $\gamma>1$, it is said to be super-diffusive; while if $\gamma <1$, it is said to be sub-diffusive \citep{Weeks1998}.
...
While earlier works have developed models of bedload diffusion, there are contradictions in the literature about the expected diffusion characteristics (super/normal/sub) of each range, and no model has been developed, to our knowledge, that derives all three diffusion ranges from process-based concepts of fluvial sediment transport.
...
\section{Bedload diffusion with trapping as a multi-state random walk}
\label{sec:model}
...
Our development of the governing equations for the three-state walk closely mimics that in \citet{Weiss1994}, and our means of incorporating the trapping process is essentially contained in \citet{Schmidt2007}. 
...
We consider burial to be a permanent condition which has some probability to occur when resting particles are covered by transported sediment.
The probability of burial per unit time (burial rate) is considered constant, so the probability of trapping increases the longer grains rest.
